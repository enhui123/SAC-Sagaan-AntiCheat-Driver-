...
PVOID ObHandle = NULL;
ULONG ProtectedProcess = 5672; // Your game's PID
...
OB_PREOP_CALLBACK_STATUS PreCallback(PVOID RegistrationContext, POB_PRE_OPERATION_INFORMATION OperationInformation)
{
	...
	if (OpenedProcess == Csrss1) // Making sure to not strip csrss's Handle, will cause BSOD
		return OB_PREOP_SUCCESS;

	if (OpenedProcess == Csrss2)// Making sure to not strip csrss's Handle, will cause BSOD
		return OB_PREOP_SUCCESS;
	...
	if (PsGetProcessId((PEPROCESS)OperationInformation->Object) == ProtectedProcess) // PsGetProcessId((PEPROCESS)OperationInformation->Object) equals to the created handle's PID, so if the created Handle equals to the protected process's PID, strip
	{
		...
		{
			if (OperationInformation->Operation == OB_OPERATION_HANDLE_CREATE) // checking what operation it is, if its create, strip
			{
			...
			else // else, strip the duplicated handle
			{
		...
		if (OperationInformation->Operation == OB_OPERATION_HANDLE_CREATE) // striping lsass.exe's handle 
		{
...
VOID EnableCallBack()
{
	...
	OBOCallbackRegistration.OperationRegistrationCount = (USHORT)2;
